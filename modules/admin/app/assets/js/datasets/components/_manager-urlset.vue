...
import {UrlSetConfig} from "../types";


export default {
  components: {FilterControl, FilesTable, FastFilesTable, PanelLogWindow, DragHandle, ModalInfo, PanelFilePreview, ButtonValidate, ButtonDelete, ModalUrlsetConfig},
  ...
  methods: {
    doSync: async function (opts: UrlSetConfig) {
      this.tab = "sync";
      this.waiting = true;
...
<template>
  <div id="urlset-manager-container" class="stage-manager-container">
    ...
    <div id="urlset-panel-container" class="panel-container">
      <div class="top-panel">
        <files-table
      ...
      <div id="urlset-status-panels" class="bottom-panel">
        <ul class="status-panel-tabs nav nav-tabs">
          ...
          <li>
            <drag-handle
                v-bind:ns="fileStage"
                v-bind:p2="() => $root.$el.querySelector('#urlset-status-panels')"
                v-bind:container="() => $root.$el.querySelector('#urlset-panel-container')"
