...
object UrlSetHarvester {
  ...
  /**
    ...
    */
  case class UrlSetJob(repoId: String, datasetId: String, jobId: String, data: UrlSetData)
    extends HarvestJob
}
...
case class UrlSetHarvester (client: WSClient, storage: FileStorage)(
  ...
  def running(job: UrlSetJob, msgTo: ActorRef, done: Int, fresh: Int, start: LocalDateTime): Receive = {
